...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
    ...
    if use_xavier:
        ...
        with tf.device('/cpu:0'):
            var = tf.compat.v1.truncated_normal(shape, stddev=np.sqrt(2 / shape[-1]))
...
def conv2d(inputs,
    ...
    with tf.compat.v1.variable_scope(scope) as sc:
        ...
        if bn:
            outputs = tf.keras.layers.BatchNormalization(momentum=0.99, epsilon=1e-6)(outputs)
        if activation_fn is not None:
...
def dropout(inputs,
    ...
    """
    with tf.compat.v1.variable_scope(scope) as sc:
        outputs = tf.keras.layers.Dropout(rate=1-keep_prob, noise_shape=noise_shape)(inputs) 
        return outputs
