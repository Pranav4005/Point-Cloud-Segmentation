...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.compat.v1.variable_scope('compat.v1.layers'):
        ...
        with tf.compat.v1.variable_scope('results'):
            self.correct_prediction = tf.nn.in_top_k(predictions=tf.cast(valid_logits, tf.float32), targets=valid_labels, k=1)
    ...
    def inference(self, inputs, is_training):
        ...
        feature = tf.keras.layers.Dense(
            8,
            activation=None,
            name='fc0'
        )(feature)

        feature = helper_tf_util.batch_norm_for_conv1d(
            feature,
            is_training,
            bn_decay=None,
            scope='bn0'
    ...
    def train(self, dataset):
        while self.training_epoch < self.config.max_epoch:
            log_out(
                '****EPOCH {}****'.format(self.training_epoch),
                self.Log_file
            )
            ...
            for step in range(self.config.train_steps):
                ...
                try:
                    ...
                    _, _, step_summary, l_out, probs, labels, acc = self.sess.run(
                        ops,
    ...
    def evaluate(self, dataset):
        ...
        for n in range(0, self.config.num_classes, 1):
            ...
            if denominator == 0:
                ...
                iou = true_positive_classes[n] / denominator
            iou_list.append(iou)
    ...
    def random_sample(feature, pool_idx):
        ...
        pool_features = tf.compat.v1.batch_gather(
            feature,
            pool_idx
        )
    ...
    def nearest_interpolation(feature, interp_idx):
        ...
        interpolated_features = tf.compat.v1.batch_gather(
            feature,
            interp_idx
        )
    ...
    def gather_neighbour(pc, neighbor_idx):
        ...
        features = tf.compat.v1.batch_gather(
            pc,
            index_input
        )
    ...
    def att_pooling(feature_set, d_out, name, is_training):
        ...
        att_activation = tf.keras.layers.Dense(
            d,
            activation=None,
            use_bias=False,
            name=name + 'fc'
